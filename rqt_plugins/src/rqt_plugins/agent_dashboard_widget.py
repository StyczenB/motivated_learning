...
import rospkg
import rospy
from robot_msgs.msg import ChargersMsg, AgentStateMsg
...
class AgentDashboardWidget(QWidget):
    ...
    def __init__(self, plugin=None):
        ...
        self._states = {State.IDLE: 'idle',
                        State.MOVING: 'moving', State.CHARGING: 'charging'}
        ...
        rp = rospkg.RosPack()
        ui_file_path = os.path.join(rp.get_path(
            'rqt_plugins'), 'resource', 'AgentDashboard.ui')
        ...
        self._pause_sim_client = rospy.ServiceProxy(
            '/gazebo/pause_physics', Empty)
        self._pause_sim_client.wait_for_service()
        self._unpause_sim_client = rospy.ServiceProxy(
        ...
        self.continousCheckBox.setChecked(
            rospy.get_param('/continuous_movement', False))
    ...
    def start(self):
        ...
        """
        self._agent_state_sub = rospy.Subscriber(
            self._topics['agent_state'], AgentStateMsg, self.agent_state_cb)
        self._chargers_sub = rospy.Subscriber(
            self._topics['chargers'], ChargersMsg, self.chargers_cb)
    ...
    def refresh_chargers_window(self, chargers_state: ChargersMsg):
        ...
        for i, charger in enumerate(chargers_state.chargers):
            self._tableChargersTopic.setItem(
                i, 0, QTableWidgetItem(charger.name))
            self._tableChargersTopic.setItem(
                i, 1, QTableWidgetItem(str(charger.coords.x)))
            self._tableChargersTopic.setItem(
                i, 2, QTableWidgetItem(str(charger.coords.y)))
            charger_value_item = QTableWidgetItem(
                f'{charger.charger_value:.4f}')
    ...
    def refresh_agent_state_window(self, agent_state: AgentStateMsg):
        ...
        self._tableAgentState.setItem(
            0, 1, QTableWidgetItem(f'{agent_state.coords}'))
        battery_level_item = QTableWidgetItem(
            f'{agent_state.battery_level:.3f}')
        ...
        self._tableAgentState.setItem(
            0, 3, QTableWidgetItem(str(agent_state.state)))
        self._tableAgentState.setItem(
            0, 4, QTableWidgetItem(self._states[agent_state.state]))
