...
class Agent:
    def __init__(self):
        ...
        self._state_pub = rospy.Publisher('agent_state', AgentStateMsg, queue_size=1)
    ...
    def step(self):
        ...
        if self._state == State.IDLE:
            ...
            if self._battery_mngr.level < BatteryManager.BATTERY_MAX:
                charger = self._chargers_mngr.get_charger(
                    x=curr_field.coords.x, y=curr_field.coords.y)
                if charger:
                    energy = charger.draw_energy()
                    self._battery_mngr.update_battery(energy)
                else:
                    rospy.logwarn(
                        f'Problem getting charger from coordinates: x={curr_field.coords.x}, y={curr_field.coords.y}')
            else:
    ...
    def get_field_from_global_map(self, x: int, y: int):
        for field in self._global_world_map.fields:
            if x == field.coords.x and y == field.coords.y:
                return field
        rospy.logwarn(
            f'Could not find valid field with coordinates: x: {x}, y: {y}')
        return None

    def get_current_field(self):
        curr_field = self.get_field_from_global_map(
            self._pos_mngr.coords.x, self._pos_mngr.coords.y)
        return curr_field
